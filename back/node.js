...
app.route({
  ...
  handler: async (request, reply) => {
    ...
    const apidata = await axios.get(
      "https://opendata.paris.fr/api/explore/v2.1/catalog/datasets/arbres-plantes-par-projet/records"
    );
    const greatdata = apidata.data;
    //console.log("Les données JSON sont : ", JSON.stringify(greatdata, null, 2));
    ...
    const response = await ollama.chat({
      ...
      messages: [
        ...
        {
          role: "tool",
          content: `Peux-tu me donner les coordonnées géographiques pour l'Alignement 2020-2021 - 05e arrondissement ? Voici les données : ${JSON.stringify(
            greatdata
          )}`,
        },
      ],
